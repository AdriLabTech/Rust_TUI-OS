...
// TUI-OS supports 3 boot protocols: rust-bootloader, limine, and multiboot2

#[cfg(not(any(feature = "limine", feature = "multiboot")))]
#[no_mangle]
extern "C" fn _start(boot_info: &'static bootloader::BootInfo) -> ! {
    tuios::sys::boot::bootloader::start(boot_info)
}

#[cfg(feature = "limine")]
#[no_mangle]
extern "C" fn _start() -> ! {
    tuios::sys::boot::limine::start()
}
...
core::arch::global_asm!(
    ...
    "call {start}",
    "hlt",
    size = const tuios::STACK_SIZE,
    start = sym start,
);

#[cfg(feature = "multiboot")]
extern "C" fn start(info: u32, magic: u32) -> ! {
    tuios::sys::boot::multiboot::start(info, magic)
}

#[panic_handler]
fn panic(info: &PanicInfo) -> ! {
    use alloc::string::ToString;
    use tuios::api::console::Style;
    use tuios::{error, hang, eprint, eprintln};
