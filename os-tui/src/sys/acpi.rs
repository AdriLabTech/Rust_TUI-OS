...
pub fn init() {
    let res = unsafe { AcpiTables::search_for_rsdp_bios(TuiosAcpiHandler) };
    match res {
        Ok(acpi) => {
            ...
            if let Ok(dsdt) = acpi.dsdt() {
                ...
                let handler = Box::new(TuiosAmlHandler);
...
#[derive(Clone)]
pub struct TuiosAcpiHandler;

impl AcpiHandler for TuiosAcpiHandler {
...
struct TuiosAmlHandler;

impl Handler for TuiosAmlHandler {
